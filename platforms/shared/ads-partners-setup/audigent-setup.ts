import { BaseServiceSetup, context, externalLogger, utils } from '@wikia/ad-engine';
...
class Audigent {
	...
	static setSegmentsInTargeting(segments) {
		utils.logger(logGroup, 'Setting segments in the targeting', segments);
		context.set('targeting.AU_SEG', segments);
	}

	static canSliceSegments(segments: string | [], limit: number): boolean {
		return limit > 0 && typeof segments !== 'string';
	}

	static trackWithExternalLoggerIfEnabled(segments: string | []) {
