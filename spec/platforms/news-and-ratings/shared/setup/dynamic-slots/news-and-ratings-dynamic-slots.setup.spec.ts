...
describe('Inserting dynamic slots on NnR', () => {
	let querySelectorAllStub, instantConfigStub;

	beforeEach(() => {
		querySelectorAllStub = global.sandbox.stub(document, 'querySelectorAll');
		instantConfigStub = global.sandbox.createStubInstance(InstantConfigService);
		instantConfigStub.get.withArgs('icInterstitial').returns(false);
	...
	it('works as expected for no slots found on the page', () => {
		...
		const dynamicSlotsSetup = new NewsAndRatingsDynamicSlotsSetup(instantConfigStub);
