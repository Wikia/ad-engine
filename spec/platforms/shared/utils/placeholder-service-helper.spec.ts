...
import { AdSlotClass } from '@wikia/core';
...
describe('placeholder service helper', () => {
	...
	describe('Ad label', () => {
		...
		it(`shouldHideAdLabel returns true if adLabel contains class .${AdSlotClass.HIDDEN_AD_CLASS}`, () => {
			expect(placeholderHelper.shouldHideAdLabel(getMockElement(true))).to.equal(false);
		});

		it(`shouldHideAdLabel returns false if adLabel does not contain class .${AdSlotClass.HIDDEN_AD_CLASS}`, () => {
