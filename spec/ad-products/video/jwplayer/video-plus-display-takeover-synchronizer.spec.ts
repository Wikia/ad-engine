...
describe('VideoPlusDisplayTakeoverSynchronizer', () => {
	beforeEach(() => {
		...
		context.set('options.video.isUAPJWPEnabled', true);
	});
	...
	it('enables sync on tentpole bundle', () => {
		context.set('options.video.isUAPJWPEnabled', 'tentpole');
		setBundles(['tentpole']);
	...
	it('enables sync on tentpoles bundle', () => {
		context.set('options.video.isUAPJWPEnabled', ['tentpole1', 'tentpole2', 'tentpole3']);
