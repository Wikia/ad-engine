...
export class PrebidNativeProviderDeprecated extends BaseServiceSetup {
	...
	renderPrebidNativeAd(adSlotName: string, data: PrebidNativeData): void {
		...
		this.fireNativeTrackers(PrebidNativeProviderDeprecated.ACTION_IMPRESSION, data);
	...
	private fireNativeTrackers(action: string, adObject: PrebidNativeData): void {
		let trackers;
		if (action === PrebidNativeProviderDeprecated.ACTION_CLICK) {
			trackers = adObject && adObject.clickTrackers;
		}
		if (action === PrebidNativeProviderDeprecated.ACTION_IMPRESSION) {
	...
	private addClickTrackers(adObject: PrebidNativeData): void {
		...
		[].slice.call(linkElements).forEach((element) => {
			element.addEventListener('click', () => {
				this.fireNativeTrackers(PrebidNativeProviderDeprecated.ACTION_CLICK, adObject);
			});
		});
	}
}

export const prebidNativeProviderDeprecated = new PrebidNativeProviderDeprecated();
