...
export class PorvataListener {
	...
	constructor(public params: PorvataListenerParams, private uapParams: UapParams) {
	...
	dispatch(eventName: string, errorCode = 0, currentAd?: google.ima.Ad): void {
		...
		if (this.params.position && eventName === PorvataListener.EVENTS.viewable_impression) {
			communicationServiceSlot.emit(this.uapParams.slotName, AdSlotEvent.VIDEO_VIEWED_EVENT);
		} else if (this.params.position && eventName === PorvataListener.EVENTS.wikiaXClick) {
			communicationServiceSlot.emit(this.uapParams.slotName, PorvataListener.EVENTS.wikiaXClick);
		}
	}
