...
import { waitForAuSegGlobalSet } from '../audigent';
...
class AudigentDeprecated extends BaseServiceSetup {
	...
	async call(): Promise<void> {
		...
		if (newIntegrationEnabled) {
			await waitForAuSegGlobalSet().then((isGlobalSet) => {
				utils.logger(
					logGroup,
					'AudigentDeprecated global variable set',
					isGlobalSet,
					window['au_seg'],
				);
				this.setup();
	...
	setup(): void {
		if (isAuSegGlobalSet()) {
			AudigentDeprecated.sliceAndSetSegmentsInTargeting();
		}
	}
	...
	private static sliceAndSetSegmentsInTargeting(): void {
		const segments = AudigentDeprecated.sliceSegments();
		AudigentDeprecated.trackWithExternalLoggerIfEnabled(segments);
		AudigentDeprecated.setSegmentsInTargeting(segments);
	}
	...
	private static sliceSegments() {
		...
		if (AudigentDeprecated.canSliceSegments(segments, limit)) {
...
export const audigentDeprecated = new AudigentDeprecated();
