...
export function initMoatTracking(a, f, c) {
	...
	listeners = [];
	moatapi = { adsManager: a, ids: f, imaSDK: !0, events: [] };
	eventMapping = {
		...
		for (h in google.ima.AdEvent.Type) {
			var l = function(b) {
				if (moatapi.sendEvent) {
					...
					moatapi.events.push({ type: eventMapping[b.type] || b.type, adVolume: a.getVolume() });
			};
			a.addEventListener(google.ima.AdEvent.Type[h], l);
			listeners.push({ type: google.ima.AdEvent.Type[h], func: l });
		}
	}
