...
const isGettingComixologyTagEnabled = utils.queryString.get('comixology-disabled') !== '1';
...
context.set('services.taxonomy.comixology.enabled', isGettingComixologyTagEnabled);
...
taxonomyService.configureComixologyTargeting().then((comixologyTag) => {
	document.getElementById('comixologyTag').innerText = JSON.stringify(comixologyTag);
});
